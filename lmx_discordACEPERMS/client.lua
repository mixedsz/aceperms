		...
		if not (roleIDs == false) then
			...
			for i = 1, #roleList do
				...
				if (ROLE_CACHE[roleList[i][1]] ~= nil) then 
					...
					discordRoleId = exports.lmx_discordAPI:FetchRoleID(roleList[i][1]);
				...
				if ROLE_MAP[tostring(discordRoleId)] ~= nil then
					if (Config.Print_Perm_Grants_And_Removals) then 
						print("[lmx_discordAPI] (" .. eventLocation .. ") Added " .. GetPlayerName(src) .. " to role group " .. roleList[i][2]);
					end
			...
			if (debugScript) then 
				print("[lmx_discordAPI DEBUG] (" .. eventLocation .. ") Player " .. GetPlayerName(src) .. " has been granted their permissions...");
			end 
			return true;
		else
			if (debugScript) then 
				print("[lmx_discordAPI DEBUG] (" .. eventLocation .. ")" .. GetPlayerName(src) .. " has not gotten permissions because we could not find their roles...");
			end
			return false;
		end
	end
    ...
    if discord then
        if not RegisterPermissions(src, 'playerConnecting') then
            if InDiscordDetector[license] == nil then 
                ...
                print("[lmx_discordAPI] Player " .. GetPlayerName(src) .. " is not in the Discord.");
                return;
            end
        else
            TriggerEvent('vMenu:RequestPermissions', src); 
        ...
        if DiscordDetector[license] == nil then 
            ...
            print('[lmx_discordAPI] Discord was not found for player ' .. GetPlayerName(src) .. "...");
            return;
        end
    end
end)